...
implements Module {
    ...
    private static ZombieZMapData.Point nearestSkull(BlockPos pos, double maxDist) {
        ...
        for (ZombieZMapData.Zone z : ZombieZMapData.ZONES) {
            for (ZombieZMapData.Point p : z.skulls()) {
                double dz;
                double dy;
                double dx = p.x() - (double)pos.getX();
                double sq = dx * dx + (dy = p.y() - (double)pos.getY()) * dy + (dz = p.z() - (double)pos.getZ()) * dz;
    ...
    public boolean guideToNearestUnvisited() {
        ...
        ZombieZMapData.Point p = this.nearestUnvisited(mc.player.getX(), mc.player.getZ());
        if (p == null) {
            return false;
        }
        this.routeZone = -1;
