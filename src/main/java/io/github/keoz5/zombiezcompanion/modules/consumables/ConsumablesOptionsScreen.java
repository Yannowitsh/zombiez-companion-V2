...
extends ModuleOptionsScreen {
    ...
    protected void initOptions() {
        ...
        this.addToggle(x, y, optionW, (Text)Text.translatable((String)"zombiezcompanion.consumables.toggle.lure"), () -> this.config().lureTimer, v -> {
            this.config().lureTimer = v;
        });
        this.addToggle(x, y + 28, optionW, (Text)Text.translatable((String)"zombiezcompanion.consumables.toggle.flower"), () -> this.config().flowerTimer, v -> {
    ...
    private void addToggle(int x, int y, int w, Text label, BoolGetter getter, BoolSetter setter) {
        this.addDrawableChild(new StyledButton(x, y, w, 20, ConsumablesOptionsScreen.toggleLabel(label, getter.get()), button -> {
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        ctx.drawTextWithShadow(this.textRenderer, (Text)Text.translatable((String)"zombiezcompanion.consumables.options.header"), this.panelX1 + 36, this.contentY1 + 12, -854792);
        ctx.drawText(this.textRenderer, (Text)Text.translatable((String)"zombiezcompanion.consumables.options.hint"), this.panelX1 + 36, this.contentY1 + 76, -8353376, false);
    }
