...
extends Screen {
    ...
    public ConfigScreen(Screen parent, ConfigManager configManager, ModuleManager moduleManager) {
        super((Text)Text.literal((String)"ZombieZ Companion"));
        this.parent = parent;
    ...
    protected void init() {
        ...
        this.addDrawableChild(new StyledButton(this.panelX2 - 12 - 18, this.titleY1 + 8, 18, 18, (Text)Text.literal((String)"X"), btn -> this.close(), -266723542, -265932737, -854792));
        int tabsX = this.mainX1 + 12;
        int tabsY = this.toolbarY1 + 7;
        tabsX += this.addTab(Text.translatable((String)"zombiezcompanion.tab.all").getString().toUpperCase(Locale.ROOT), null, tabsX, tabsY) + 6;
        ...
        this.searchField = new TextFieldWidget(this.textRenderer, searchX, searchY, 156, 22, (Text)Text.literal((String)""));
        this.searchField.setMaxLength(64);
        this.searchField.setPlaceholder((Text)Text.translatable((String)"zombiezcompanion.search.placeholder"));
        ...
        this.addDrawableChild(new StyledButton(this.panelX1 + 12, btnY, 92, btnH, this.debugLabel(), btn -> {
            boolean next;
        ...
        this.addDrawableChild(new StyledButton(this.panelX1 + 12 + 92 + 6, btnY, 78, btnH, (Text)Text.translatable((String)"zombiezcompanion.keybinds.open"), btn -> {
            if (this.client != null) {
                this.client.setScreen((Screen)new KeybindsScreen(this));
            }
        }, -266723542, -265932737, -854792));
        this.addDrawableChild(new StyledButton(this.panelX1 + 12 + 92 + 6 + 78 + 6, btnY, 90, btnH, (Text)Text.translatable((String)"zombiezcompanion.hud.editor.open"), btn -> {
            if (this.client != null) {
                this.client.setScreen((Screen)new HudEditorScreen(this, this.configManager));
            }
        }, -266723542, -265932737, -854792));
        this.addDrawableChild(new StyledButton(this.panelX2 - 12 - 92 - 6 - 78, btnY, 78, btnH, (Text)Text.translatable((String)"zombiezcompanion.title.button.discord"), btn -> Util.getOperatingSystem().open(URI.create("https://discord.gg/mqJp9CKcEX")), -11441921, -8874241, -854792));
        this.addDrawableChild(new StyledButton(this.panelX2 - 12 - 92 - 6 - 78 - 6 - 88, btnY, 88, btnH, (Text)Text.translatable((String)"zombiezcompanion.feedback.button"), btn -> {
            if (this.client != null) {
                this.client.setScreen((Screen)new FeedbackScreen(this, this.configManager));
            }
        }, -266723542, -265932737, -854792));
        this.addDrawableChild(new StyledButton(this.panelX2 - 12 - 92, btnY, 92, btnH, (Text)Text.translatable((String)"zombiezcompanion.button.close"), btn -> this.close(), -266723542, -265932737, -854792));
    }
    ...
    private Text debugLabel() {
        ...
        return Text.translatable((String)(on ? "zombiezcompanion.debug.label.on" : "zombiezcompanion.debug.label.off"));
    }

    private int addTab(String label, ModuleCategory cat, int x, int y) {
        int w = Math.max(40, this.textRenderer.getWidth(label) + 14);
        this.addDrawableChild(new CategoryTabButton(x, y, w, 22, (Text)Text.literal((String)label), btn -> {
    ...
    private void layoutCards() {
        for (CardLayout c : this.cards) {
            this.remove((Element)c.optionsBtn);
            this.remove((Element)c.toggleBtn);
        }
        this.cards.clear();
    ...
    private CardLayout buildCard(Module m, int x, int y) {
        ...
        int toggleY = y + 92 - 18;
        StyledButton options = new StyledButton(btnX, optionsY, btnW, 16, (Text)Text.translatable((String)"zombiezcompanion.card.options"), btn -> this.openOptions(m), -266723542, -265932737, -854792);
    ...
    private static Text toggleLabel(boolean enabled) {
        return Text.translatable((String)(enabled ? "zombiezcompanion.card.toggle.on" : "zombiezcompanion.card.toggle.off"));
    }

    private void toggleModule(Module m, ButtonWidget btn) {
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < this.cards.size(); ++i) {
            ...
            if (yOff != 0) {
                ...
                ctx.getMatrices().translate(0.0f, (float)yOff, 0.0f);
            }
        ...
        if (this.cards.isEmpty()) {
            ...
            MutableText primary = Text.translatable((String)(noModules ? "zombiezcompanion.empty.no_modules.primary" : "zombiezcompanion.empty.no_match.primary"));
            MutableText hint = Text.translatable((String)(noModules ? "zombiezcompanion.empty.no_modules.hint" : "zombiezcompanion.empty.no_match.hint"));
            int cx = (this.mainX1 + this.panelX2) / 2;
            int cy = (this.contentY1 + this.contentY2) / 2 - 6;
            ctx.drawCenteredTextWithShadow(this.textRenderer, (Text)primary, cx, cy, -8353376);
            ctx.drawCenteredTextWithShadow(this.textRenderer, (Text)hint, cx, cy + 12, -12235684);
        }
    ...
    private void renderCardTooltip(DrawContext ctx, int mouseX, int mouseY) {
        ...
        for (CardLayout c : this.cards) {
            ...
            int tw = this.textRenderer.getWidth(desc);
            ...
            ctx.drawBorder(tx, ty, boxW, boxH, -8874241);
            ctx.drawText(this.textRenderer, this.textRenderer.trimToWidth(desc, boxW - 12), tx + 6, ty + 5, -854792, false);
            return;
        }
    }

    private void renderForegroundText(DrawContext ctx) {
        int logoX = this.panelX1 + 12;
        int logoY = this.titleY1 + 8;
        this.drawLogo(ctx, logoX, logoY);
        int titleX = logoX + 30;
        ctx.drawText(this.textRenderer, (Text)Text.literal((String)"ZOMBIEZ"), titleX, this.titleY1 + 7, -854792, true);
        ctx.drawText(this.textRenderer, (Text)Text.literal((String)"COMPANION"), titleX + 62, this.titleY1 + 7, -8874241, true);
        MutableText version = Text.translatable((String)"zombiezcompanion.header.version", (Object[])new Object[]{ConfigScreen.currentVersion()});
        int vw = this.textRenderer.getWidth((StringVisitable)version);
        ...
        ctx.drawText(this.textRenderer, (Text)version, titleX + 5, this.titleY1 + 21, -8874241, false);
        ctx.enableScissor(this.panelX1, this.contentY1, this.panelX2, this.contentY2);
        ...
        for (int i = 0; i < this.cards.size(); ++i) {
            ...
            ctx.drawCenteredTextWithShadow(this.textRenderer, (Text)Text.literal((String)c.module.displayName()), x, c.y + 34, -854792);
            if (yOff == 0) continue;
            ctx.getMatrices().pop();
        }
        ctx.disableScissor();
    ...
    private void drawLogo(DrawContext ctx, int x, int y) {
        int s = 22;
    ...
    private void drawModuleGlyph(DrawContext ctx, Module module, int centerX, int centerY) {
        int w = 38;
        int h = 26;
        int x = centerX - w / 2;
        int y = centerY - h / 2;
        ...
        int textW = this.textRenderer.getWidth(glyph);
        ctx.drawTextWithShadow(this.textRenderer, (Text)Text.literal((String)glyph), centerX - textW / 2 + 1, centerY - 4, -8874241);
    }
