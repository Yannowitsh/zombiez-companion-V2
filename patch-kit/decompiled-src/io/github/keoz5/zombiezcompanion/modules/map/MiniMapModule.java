...
implements Module {
    ...
    private class_304 peekKey;

    public class_304 peekKey() {
        return this.peekKey;
    }
    ...
    public String description() {
        return class_2561.method_43471((String)"zombiezcompanion.module.mini_map.desc").getString();
    }
    ...
    public void onRegister(ModuleContext ctx) {
        ...
        this.peekKey = KeyBindingHelper.registerKeyBinding((class_304)new class_304("key.zombiezcompanion.minimap_peek", class_3675.class_307.field_1668, 79, "key.categories.zombiezcompanion"));
    }
    ...
    public void onHudRender(class_332 ctx, float tickDelta) {
        ...
        boolean bl = peeking = this.peekKey != null && this.peekKey.method_1434();
        if (peeking) {
    ...
    private void renderMiniMap(class_332 ctx, class_310 client, MapConfig cfg, int x, int y, int size) {
        double zoom = Math.max(0.25, Math.min(2.0, cfg.miniMapZoom));
        double centerX = ZombieZMapData.mapX(client.field_1724.method_23317());
        double centerY = ZombieZMapData.mapY(client.field_1724.method_23321());
        ...
        ctx.method_25294(x + 1, y + 2, x + size + 2, y + size + 3, -1442840576);
        ctx.method_25294(x - 1, y - 1, x + size + 1, y + size + 1, -15658216);
        ctx.method_49601(x - 1, y - 1, size + 2, size + 2, -8874241);
        ctx.method_44379(x, y, x + size, y + size);
        ...
        MiniMapModule.drawHeadingArrow(ctx, px, py, client.field_1724.method_36454());
        this.renderMiniWaypoints(ctx, client, cfg, x, y, size, viewX, viewY, zoom);
        ctx.method_44380();
    }

    private void renderMiniPresences(class_332 ctx, int screenX, int screenY, int size, double viewX, double viewY, double zoom) {
        ...
        for (PresenceCache.Presence p : list) {
            ...
            ctx.method_25294(px - 2, py - 2, px + 3, py + 3, -587202560);
            ctx.method_25294(px - 1, py - 1, px + 2, py + 2, -13058568);
        }
    }

    private void renderTiles(class_332 ctx, int screenX, int screenY, int size, double viewX, double viewY, double zoom) {
    ...
    private void renderMiniMarkers(class_332 ctx, MapConfig cfg, int screenX, int screenY, int size, double viewX, double viewY, double zoom) {
        class_310 client = class_310.method_1551();
        ...
        if (cfg.showZones) {
            for (ZombieZMapData.Zone zone : ZombieZMapData.ZONES) {
                ...
                if (yMin >= screenY && yMin <= screenY + size) {
                    ctx.method_25294(screenX, yMin, screenX + size, yMin + 1, color);
                }
                if (yMax >= screenY && yMax <= screenY + size) {
                    ctx.method_25294(screenX, yMax, screenX + size, yMax + 1, color);
                }
                ...
                ctx.method_27535(mc.field_1772, (class_2561)class_2561.method_43470((String)label), screenX + 3, labelY, textColor);
            }
        }
        if (cfg.showRefuges) {
            int fill = 1143842406;
            ...
            for (ZombieZMapData.Refuge refuge : ZombieZMapData.REFUGES) {
                ...
                ctx.method_25294(lx1, ly1, lx2, ly2, fill);
                if (y1 >= screenY && y1 <= screenY + size) {
                    ctx.method_25294(lx1, y1, lx2, y1 + 1, border);
                }
                if (y2 >= screenY && y2 <= screenY + size) {
                    ctx.method_25294(lx1, y2 - 1, lx2, y2, border);
                }
                if (x1 >= screenX && x1 <= screenX + size) {
                    ctx.method_25294(x1, ly1, x1 + 1, ly2, border);
                }
                if (x2 < screenX || x2 > screenX + size) continue;
                ctx.method_25294(x2 - 1, ly1, x2, ly2, border);
            }
        }
    ...
    private void renderMiniWaypoints(class_332 ctx, class_310 client, MapConfig cfg, int screenX, int screenY, int size, double viewX, double viewY, double zoom) {
    ...
    private void drawMiniMarker(class_332 ctx, int screenX, int screenY, int size, double viewX, double viewY, double zoom, double worldX, double worldZ, int rgb, int radius) {
        ...
        ctx.method_25294(x - radius - 1, y - radius - 1, x + radius + 2, y + radius + 2, -1442840576);
        ctx.method_25294(x - radius, y - radius, x + radius + 1, y + radius + 1, 0xFF000000 | rgb & 0xFFFFFF);
    }

    private void drawMiniSmartMarker(class_332 ctx, int screenX, int screenY, int size, double viewX, double viewY, double zoom, double worldX, double worldZ, int rgb, int radius) {
    ...
    private void drawMiniDiamond(class_332 ctx, int x, int y, int radius, int color) {
    ...
    private static void drawHeadingArrow(class_332 ctx, int px, int py, float yaw) {
        ctx.method_51448().method_22903();
        ctx.method_51448().method_46416((float)px, (float)py, 0.0f);
        ctx.method_51448().method_22907(class_7833.field_40718.rotationDegrees(yaw + 180.0f));
        ctx.method_25294(-1, -5, 2, 4, -872415232);
        ctx.method_25294(-3, -3, 4, -1, -872415232);
        ctx.method_25294(0, -4, 1, 3, -1);
        ctx.method_25294(-2, -2, 3, -1, -1);
        ctx.method_25294(0, -3, 1, 2, -8874241);
    ...
    private ZombieZMapData.Zone playerZone(class_310 client) {
        if (client == null || client.field_1724 == null) {
            return null;
        }
        return this.zoneForZ(client.field_1724.method_23321());
    }
