...
public final class Commands {
    ...
    public static void register(ConfigManager configManager, ModuleManager moduleManager) {
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, access) -> dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)ClientCommandManager.literal((String)"zzc").then(ClientCommandManager.literal((String)"debug").executes(ctx -> {
            ...
            configManager.save();
            ((FabricClientCommandSource)ctx.getSource()).sendFeedback((class_2561)class_2561.method_43469((String)"zombiezcompanion.command.debug.state", (Object[])new Object[]{Commands.stateText(next)}));
            Log.info("Debug mode " + (next ? "ON" : "OFF"));
            return 1;
        }))).then(ClientCommandManager.literal((String)"status").executes(ctx -> {
            StringBuilder sb = new StringBuilder();
            sb.append(class_2561.method_43471((String)"zombiezcompanion.command.status.header").getString()).append('\n');
            for (Module m : moduleManager.modules()) {
                sb.append(class_2561.method_43469((String)"zombiezcompanion.command.status.module_line", (Object[])new Object[]{m.id(), Commands.stateText(moduleManager.isEnabled(m.id()))}).getString()).append('\n');
            }
            sb.append(class_2561.method_43469((String)"zombiezcompanion.command.status.debug_line", (Object[])new Object[]{Commands.stateText(configManager.get().debugMode)}).getString());
            ...
            configManager.save();
            ((FabricClientCommandSource)ctx.getSource()).sendFeedback((class_2561)class_2561.method_43471((String)"zombiezcompanion.command.reload.saved"));
            return 1;
        }))));
    }

    private static String stateText(boolean enabled) {
        return class_2561.method_43471((String)(enabled ? "zombiezcompanion.command.state.on" : "zombiezcompanion.command.state.off")).getString();
    }
}
